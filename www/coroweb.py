...
def get(path):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kw):
            # logging.info('get 装饰器预先给fn添加了method和path信息')
            # logging.info('fn-method: ',wrapper.__method__)
            # logging.info('fn-route: ', wrapper.__route__)
            # logging.info('可看到此fn的装饰器预定义与request的形式吻合，于是调用fn')
            return func(*args, **kw)
